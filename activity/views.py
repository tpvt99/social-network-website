    ...
    def post(self, request):
        if request.is_ajax():
            ...
            if activity.privacy == 'public':
                status = Status.objects.create(user = request.user, status_type = 'create-activity', activity = activity, privacy = 'public')
            else:
                status = Status.objects.create(user = request.user, status_type = 'create-activity', activity = activity, privacy = activity.privacy)
    ...
    def post(self, request):
        ...
        if request.user == activity.user:
            ...
            for i in ap:
                ...
                status = Status.objects.create(user = i.person, status_type = 'activitypost', privacy = 'private', activitypost = activitypost)
                if status.user == activity.user:
                    ttt.append(status)
