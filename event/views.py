...
class CreateEvent(Helper):
    ...
    def post(self, request):
        ...
        status = Status.objects.create(user = request.user, status_type = 'create-event', event = event, privacy = event.privacy)
    ...
    def post(self, request):
        ...
        if request.user == event.user:
            ...
            for i in ep:
                ...
                status = Status.objects.create(user = i.user, status_type = 'eventpost', privacy ='private', eventpost = eventpost)
                if status.user == event.user:
                    ttt.append(status)
