...
KIND_OF_NOTIFICATION = {
        ...
        'status':'status',
        'status-a':'somebody feel cool your status',
...
class EventDNotification(models.Model):
    noti = models.ForeignKey(Notification, on_delete = models.CASCADE)
    event = models.ForeignKey(Event, on_delete = models.CASCADE)
    guess = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete = models.CASCADE, related_name = "%(app_label)s_%(class)s_guess", null = True)
    time = models.DateTimeField(auto_now_add = True, null = True)
    time_send = models.DateTimeField(null = True)
...
class StatusANotification(models.Model):
    ...
    who_vote = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete = models.CASCADE, related_name = "%(app_label)s_%(class)s_user", null = True)

class StatusBNotification(models.Model):
    noti = models.OneToOneField(Notification, on_delete = models.CASCADE)
    status = models.ForeignKey(Status, on_delete = models.CASCADE)
    statuscomment = models.ForeignKey(StatusComment, on_delete = models.CASCADE)
